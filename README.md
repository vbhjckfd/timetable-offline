...
Route names arrive in Cyrillic and have to map onto icon filenames. `А03` → `a03` → `a3` → `3` → `3a.svg`; `Т25` → `t25.svg`; `Аеропорт` → `airport.svg`. The rules live in `get_transfers`, and every route the API currently serves resolves to a real file.
